...
def get_bfe_secret_v3():
    ...
    try:
        ...
        if(json2["code"]=="200"):
            ...
            return secret,x_bfe_session_id
        else:
...
def uploadprogress_v3(secret,x_bfe_session_id,begin_time,report_time,lesson_id,course_id,i):
    ...
    url = "https://bfe.ewt360.com/monitor/web/collect/batch?"+f'TrVideoBizCode=1001&TrFallback=0&TrUserId={userid}&TrLessonId={lesson_id}&TrUuId={uuid}&sdkVersion=3.0.5&_={_ts}'
    header = {
...
def run(cls,day):
    ...
    times = round(lesson_duration/60)+2
    print("START ==",cls["title"],lesson_id,course_id)
    secret,x_bfe_session_id = get_bfe_secret_v3()
    now = math.floor(time.time()*1000)
    begin_time = now - times*60000 - random.randint(160000,10000000)
    for i in range(1,times+2):
        report_time  = begin_time + random.randint(700,5000) + (i-1)*60000 #now-(50-i)*60000
